...
export default function Order({ order }: { order: order }) {
  ...
  return (
    <div className="w-full rounded overflow-hidden border" key={1}>
      {/* Order Header */}
      <div className="w-full bg-gray-200 text-sm p-1.5 flex justify-between px-3">
        <span>
          Order No.{" "}
          <Link className="underline" href={"/account/orders/" + order.number}>
            {order.number}
          </Link>
        </span>
        <span>
      ...
      <div className="flex gap-3 p-3 items-start">
        ...
        <div className="basis-5/6 flex flex-col gap-0">
          ...
          <span className="text-blue-800 text-sm">
            <Link href={"/users/" + order.listing.user.username}>
